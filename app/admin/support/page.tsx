...
export default async function AdminSupportPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <AdminShell activePage="support">
      <div>
        <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">Customer operations</p>
        <h1 className="mt-2 text-4xl font-black text-slate-950">Support</h1>
        <p className="mt-2 text-slate-600">Prioritize open customer issues and find requests by order, email or ICCID.</p>
      </div>

      <div className="mt-6 flex flex-wrap gap-3">
        <span className="rounded-xl bg-blue-50 px-4 py-3 font-black text-blue-700">{open} open</span>
        <span className="rounded-xl bg-amber-50 px-4 py-3 font-black text-amber-700">{inProgress} in progress</span>
      </div>
            ...
            <Link key={request.id} href={`/admin/support/${request.id}`} className="grid gap-4 px-5 py-5 hover:bg-slate-50 md:grid-cols-[1.3fr_1fr_1fr_auto] md:items-center">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className={`rounded-full px-3 py-1 text-xs font-black ${statusClass(request.status)}`}>{request.status.replace("_", " ")}</span>
