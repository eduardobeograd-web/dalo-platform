...
export default async function DestinationAtAGlance({
  ...
  const exchangeRate = essentials.currencyCode
    ...
    : null;
  const hasEmergencyGuidance = Boolean(
    essentials.emergencyNumbers &&
      essentials.emergencyLabel &&
      essentials.emergencySourceUrl,
  );
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-5 pb-12 sm:px-7 lg:px-8">
      <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_22px_60px_-42px_rgba(15,23,42,0.45)]">
        ...
        <div
          className={`grid divide-y divide-slate-100 sm:grid-cols-2 sm:divide-x lg:divide-y-0 ${
            hasEmergencyGuidance ? "lg:grid-cols-4" : "lg:grid-cols-3"
          }`}
        >
        ...
        <p className="border-t border-slate-100 px-6 py-3 text-xs leading-5 text-slate-500 lg:px-8">
          Times and rates are travel guidance. Check local instructions when you
          arrive; large destinations may use several time zones, and card or cash
          rates can differ. Emergency numbers are shown only when an official
